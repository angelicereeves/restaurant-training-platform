...
export default async function ModulePage({
  ...
  const moduleLessons = lessons.filter((l) => l.moduleId === trainingModule.id)
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-8">
      <div className="mx-auto max-w-4xl">
        ...
        <p className="mt-2 text-gray-700">{trainingModule.description}</p>
        ...
        <div className="mt-8 space-y-4">
          {moduleLessons.map((lesson) => (
            ...
            >
              ...
              <p className="mt-2 text-sm text-gray-600">Open lesson →</p>
            </Link>
          ))}

          {moduleLessons.length === 0 && (
            <p className="text-gray-600">No lessons in this module yet.</p>
          )}
        </div>
      </div>
    </main>
